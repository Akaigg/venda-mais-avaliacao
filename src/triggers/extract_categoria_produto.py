...
def extract_categoria_produto(myTimer: func.TimerRequest) -> None:
    ...
    query_extract = f"SELECT {colunas_sem_id} FROM [db-vendasmais-das2b].dbo.categoria_produto"
    query_load = f"INSERT INTO [db-vendasmais-das2b].dbo.categoria_produto ({colunas_sem_id}) VALUES (%s, %s, %s, %s, %s, %s, %s)"
    ...
    try:
        ...
        with pymssql.connect(server=sql_server_dest, user=sql_user_dest, password=sql_pass_dest, database=sql_database_dest) as conn_dest:
            with conn_dest.cursor() as cursor_dest:
                cursor_dest.execute("TRUNCATE TABLE [db-vendasmais-das2b].dbo.categoria_produto")
